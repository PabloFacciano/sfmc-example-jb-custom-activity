// JOURNEY BUILDER CUSTOM ACTIVITY - telegram-message ACTIVITY
// ````````````````````````````````````````````````````````````
...
// setup the telegram-message example app
module.exports = function discountCodeExample(app, options) {
    const moduleDirectory = `${options.rootDirectory}/modules/telegram-message`;

    // setup static resources
    app.use('/modules/telegram-message/dist', express.static(`${moduleDirectory}/dist`));
    app.use('/modules/telegram-message/images', express.static(`${moduleDirectory}/images`));

    // setup the index redirect
    app.get('/modules/telegram-message/', function(req, res) {
        return res.redirect('/modules/telegram-message/index.html');
    });

    // setup index.html route
    app.get('/modules/telegram-message/index.html', function(req, res) {
    ...
    app.get('/modules/telegram-message/config.json', function(req, res) {
     ...
     */
    app.post('/modules/telegram-message/save', function(req, res) {
        console.log('debug: /modules/telegram-message/save');
     ...
     * 50x - Return if the configuration is invalid (this will block the publish phase)
     */
    app.post('/modules/telegram-message/publish', function(req, res) {
        console.log('debug: /modules/telegram-message/publish');
        return res.status(200).json({});
    });
     ...
     * 50x - Return if the configuration is invalid (this will block the publish phase)
     */
    app.post('/modules/telegram-message/validate', function(req, res) {
        console.log('debug: /modules/telegram-message/validate');
        return res.status(200).json({});
    });
     ...
     * @return {[type]}
     */
    app.post('/modules/telegram-message/stop', function(req, res) {
        console.log('debug: /modules/telegram-message/stop');
        return res.status(200).json({});
    });
     ...
     */
    app.post('/modules/telegram-message/execute', function(req, res) {
        console.log('debug: /modules/telegram-message/execute');
